...
class Emitter:
    ...
    def get_jvm_type(self, in_type) -> str:
        ...
        if type_in is IntType:
            ...
            # Default case - return object type
    ...
    def emit_push_iconst(self, in_: Union[int, str], frame) -> str:
        ...
        if type(in_) is int:
            ...
            if in_ == "true":
                frame.pop()  # Undo the push since we're calling recursively
    ...
    def emit_re_op(self, op: str, in_, frame) -> str:
        ...
        if type(in_) is IntType or type(in_) is BoolType:
            ...
            frame.push()  # fcmpl pushes one result
            if op == ">":
            ...
            frame.pop()  # consumed by conditional jump
    ...
    def get_const(self, ast) -> tuple:
        ...
        if type(ast) is IntegerLiteral:
            ...
            return (str(ast.value).lower(), BoolType())
        elif type(ast) is StringLiteral:
            return ('"' + ast.value + '"', StringType())
        else:
            raise IllegalOperandException(f"Unsupported literal type: {type(ast)}")
    ...
    def emit_return(self, in_, frame) -> str:
        ...
        if type(in_) is IntType or type(in_) is BoolType:
            ...
            # Handle array types and other reference types
            frame.pop()
            return self.jvm.emitARETURN()
    ...
    def emit_anew_array(self, lexeme: str) -> str:
        """
        Emit ANEWARRAY instruction.
    ...
    def emit_aastore(self, frame) -> str:
        """
        Emit AASTORE instruction.
    ...
    def emit_arraylength(self, frame) -> str:
        """
        ...
        # Pop array reference, push length (int)
        # No net change in stack size
        return "\tarraylength\n"

    def emit_string_concat(self, frame) -> str:
        """
        ...
        Returns:
            Generated JVM instruction string
        """
        frame.pop()  # Pop second string
        # First string remains on stack, result pushed
        return self.jvm.emitINVOKEVIRTUAL("java/lang/String/concat", "(Ljava/lang/String;)Ljava/lang/String;")

    def emit_invokestatic(self, method_name: str, param_type, return_type, frame) -> str:
        """
        Emit INVOKESTATIC instruction for conversion functions.
        
        Args:
            method_name: Method name (e.g., "io/int2str")
        ...
        Returns:
            Generated JVM instruction string
        """
        frame.pop()  # Pop parameter
        frame.push()  # Push return value
        param_sig = self.get_jvm_type(param_type)
        return_sig = self.get_jvm_type(return_type)
        return self.jvm.emitINVOKESTATIC(method_name, f"({param_sig}){return_sig}")
